...
int main(int argc, char *argv[])
{
    ...
    char buffer[1024];              // the communciation buffer stores 1024 chars
    ...
    while(1)
    {
        ...
        case 0:
            ...
            if(strncmp(buffer,"CIPHERTEXT",10) != 0)
            {
                // server mismatch check send message warning client and exit
                sprintf(buffer,"!!!!!!!!!!!!!!!!!!!!");
                if(send(connectionSocket, buffer, 20, 0)<0)
                {
                    fprintf(stderr,"SERVER: Could not return an error message to the client\n");
                }                
                return -1;
            }            
            msgSize = atoi(&buffer[11]);    
            ...
            // handles the instance where the key did not fit on the buffer, like in plaintext4
            if(k < msgSize)
            {
                while((charsRead = recv(connectionSocket, buffer, 1024, 0)) > 0)
                {
                    ...
                    {
                        if(p<=msgSize)                  // if we have not finised out message, keep reading
                        {
                            plainText[p] = buffer[i];
                            p++;
                        }
                        else                            // otherwise start processing the key
                        {
                            keyText[k] = buffer[i];
                            k++;
                    ...
                    if(k >= msgSize)                    // once we have enough key to process the message, then we can exit loop
                    {
                        break;
                    }
                }
            ...
            {   
                char cp = plainText[i];     // on the decryption server, plainText currently stores cipherText, but will be mutated in place
                ...
                char cc = (cp-ck)%27;       // subtracting since we are decrypting
                if (cc < 0)
                {
                    cc += 27;               // if its less than, we need to add 27 (because of ' ')
                }
                if(cc == 26)
                {
                    cc = ' ';               // edge case for a ' '
