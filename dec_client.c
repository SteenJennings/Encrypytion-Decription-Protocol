...
int main(int argc, char *argv[]) {
  ...
  cipherTextData = readFile(argv[1], &cipherTextSize);

  // Strip trailing newline
  if(cipherTextData[cipherTextSize-1] == '\n')
  {
      cipherTextData[cipherTextSize-1] = '\0';
      cipherTextSize--;
  }
  ...
  keyData[cipherTextSize] = '\0';  // NULL terminate our key data at the cipherText Message Size

  if(validateFile(cipherTextData,cipherTextSize) || validateFile(keyData,cipherTextSize))
  {
      // Bad data in cipherText file or the keyFile
      return -1;
  }
  ...
  if (connect(socketFD, (struct sockaddr*)&serverAddress, sizeof(serverAddress)) < 0){
   ...
   sprintf(buffer,"CIPHERTEXT %ld",cipherTextSize);
   ...
   int n = 0;
   while(n <= cipherTextSize) 
   {
       int len = 1024;
       if(n+len > cipherTextSize)
       {
           len = cipherTextSize - n + 1;  // +1 gets our \0
       }
       charsWritten = send(socketFD, &cipherTextData[n], len, 0);   // sending the cyphertext first
       if (charsWritten < 0){
          error("CLIENT: ERROR writing to socket");
       }
       n += len;
   ...
   while(n <= cipherTextSize) 
   {
       ...
       charsWritten = send(socketFD, &keyData[n], len, 0);          // once we have sent the ciphertext, we then send the key data
       if (charsWritten < 0){
          error("CLIENT: ERROR writing to socket");
       }
       n += len;
  ...
  while((charsRead = recv(socketFD, buffer, sizeof(buffer) - 1, 0)) > 0)
  {
      if(buffer[0] == '!')
      {
          fprintf(stderr, "Error: dec_client can not use enc_server\n");
          break;
      }
